import {
  ...
  const ChatRooms = () => {
    ...
    return (
      <>
                {page===1 ?<ScrollView>{rooms.map((item, key) => {
          let a = item;
          if (a !== null) {
            ...
            return (
              <TouchableOpacity                 onPress={()=>{setName(displayName),setpage(2)}}>
              {/* <Link to="/user"> */}
                <View
                  style={{
                    flexDirection: "row",
        ...
        })}</ScrollView>: name &&<View style={{ height:"100%", flex: 1,}}>
        <ImageBackground source={image} resizeMode="cover" style={{ flex: 1, justifyContent: "center" }}>
          <View style={{ backgroundColor: '#eee', padding: 20, display: "flex", flexDirection: "row" }}>

            <Icon onPress={()=>{setpage(1)}} name="ios-arrow-back" size={30} />
